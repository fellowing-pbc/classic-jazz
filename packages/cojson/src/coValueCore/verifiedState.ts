...
import { NativeResult, parseNativeResult } from "../crypto/nativeResult.js";
...
export class VerifiedState {
  ...
  *sessionEntries(): IterableIterator<[SessionID, SessionLog]> {
  ...
  /** Whether the native backend implements the `contentToSend` decision. */
  supportsNativeContentDecision(): boolean {
    return typeof this.nodeCore.contentToSend === "function";
  }

  /**
   * Native reproduction of {@link newContentSince} (default-on production path).
   * Calls the Rust `content_to_send` with the SAME known state and returns the
   * unified {@link NativeResult}: on success `value` is the content messages, or
   * `null` when there is nothing to send; on error the caller
   * (`sync.ts`'s `#sendNewContent`) logs it and falls back to the TS path.
   */
  newContentSinceNative(
    knownState: CoValueKnownState | undefined,
  ): NativeResult<NewContentMessage[] | null> {
    const json = this.nodeCore.contentToSend?.(
      this.id,
      knownState ? JSON.stringify(knownState) : undefined,
    );
    return parseNativeResult<NewContentMessage[] | null>(json);
  }
